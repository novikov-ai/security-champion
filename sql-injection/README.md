...
func (s *Server) FilterPayments(w http.ResponseWriter, req *http.Request) {
	...
	rows, err := s.DB.Query("SELECT id, amount, user_id, description FROM payments " + "WHERE user_id = " + currentUserID + " AND amount > " + amount)
	if err == nil {
		for rows.Next() {
			var payment Payment
...
### Let's fix it
- [Add placeholders for user input](main.go#L27)